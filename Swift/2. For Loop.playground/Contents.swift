...
for values in 11...20 { // supposed to loop 10 times
    ...
    print("\(values)  ") // String Interpolation
...
//... Example 5.0
for counter in stride(from: 11, through: 1, by: -2) { // THROUGH INCLDUED
    print("\(counter) ")
}

//... Example 6.0 - Displays values from 10 to 40 by the increment of 10, not including 50
for numbers in stride(from: 10, to: 50, by: 10) { // TO NOT INCLUDED
    print("\(numbers) ")
}
//... Example 8.0
...
for ch: Character in mutablePlayground // : Character is optional, SWIFT is SMART
{
    print("\(ch)")
}
